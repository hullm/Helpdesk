...
Dim objCheckIns, bolShowLogout, intZoom, intInputSize, objUsersSet
...
'Build the SQL string for the create a ticket drop down
strSQL = "SELECT DISTINCT Name" & vbCRLF
strSQL = strSQL & "FROM Main" & vbCRLF
strSQL = strSQL & "ORDER BY Name" & vbCRLF

'Execute the SQL string
Set objUsersSet = Application("Connection").Execute(strSQL)
...
<%Sub MainVersion 
   ...
   <% If Not objUsersSet.EOF Then %>
   	<script>
   	
   		$(function() {
   			var possibleUsernames = [
   		<% Do Until objUsersSet.EOF 
					If Not IsNumeric(Left(objUsersSet(0),2)) Then 
						If InStr(objUsersSet(0)," ") = 0 Then %>	
							"<%=LCase(objUsersSet(0))%>",
				<% 	End If
					End If
					objUsersSet.MoveNext 
   			Loop %>
   		];
			$( "#usernames" ).autocomplete({
				source: possibleUsernames
			});
   	});
   	</script>
   <% End If %>
...
<%          End If %>  
      ...
      <% If strRole <> "Data Viewer" AND Application("UseAD") Then %>       
            ...
            <form method="POST" action="index.asp">		
               ...
               <tr>
                  <td>
                     Username: <input type="text" id="usernames" name="UserName" value="<%=strUserName%>" size="25">
                  </td>
                  <td>
      ...
      <center><b><%=Application("SchoolName")%> Help Desk Admin</b></center>
   	<div>
   		<ul class="NavBar" align="center">
   			<li><a class="NavBarCurrent" href="index.asp">Home</a></li>
...
<%    If objMessage(4) And (objMessage(1) = "Both" Or objMessage(1) = "Techs") And (objMessage(3) = "Top" or objMessage(3) = "Both") Then %>
      ...
      <% Select Case objMessage(2)
            ...
            Case "Alert"
               strMessageFont = "<font class=""information"">"
         End Select%>
            <center><%=strMessageFont%><%=Replace(objMessage(0),vbCRLF,"<br />")%> </font></center>
         </div>
<%    End If %>   
      <div class="TwoColumnCard">

      <form method="POST" action="view.asp">
      	<div class="TwoColumnCardTitle">Search for a Ticket</div>
         <div>
            <div Class="TwoColumnCardColumn1">Status: </div>
				<div Class="TwoColumnCardColumn2">
               <select Class="TwoColumnCard" name="Status">			
                  <option>Any</option>
                  <option selected="selected">Any Open Ticket</option>
         ...
         <div>
            <div Class="TwoColumnCardColumn1">Category: </div>
            <div Class="TwoColumnCardColumn2">
         ...
         <div>
            <div Class="TwoColumnCardColumn1">Tech: </div>     
            <div Class="TwoColumnCardColumn2">
         ...
         <div>
            <div Class="TwoColumnCardColumn1">Sort By: </div>
				<div Class="TwoColumnCardColumn2">
               <select Class="TwoColumnCard" size="1" name="Sort">
                  <option>Date - Newest on Top</option>
                  <option>Date - Oldest on Top</option>
                  <option>Location - A to Z</option>
                  <option>Location - Z to A</option>
         ...
         <div>
            <div Class="TwoColumnCardColumn1">Viewed: </div>
            <div Class="TwoColumnCardColumn2">
               <select Class="TwoColumnCard" size="1" name="Viewed">
                  <option>Any</option>
                  <option>Yes</option>
                  <option>No</option>
               </select>
         ...
         <div>
            <div Class="TwoColumnCardColumn1">Submitted:</div>
            <div Class="TwoColumnCardColumn2">
               <select Class="TwoColumnCard" size="1" name="Days">
                  <option value="0">Any</option>
                  <option value="1">Today</option>
                  <option value="7">Within the Past Week</option>
                  <option value="14">Within the Past 2 Weeks</option>
         ...
         <div>
         	<div Class="TwoColumnCardColumn1">User: </div>
         	<div Class="TwoColumnCardColumn2">
         		<input Class="TwoColumnCard" type="text" name="User" value="Any" size="<%=intInputSize%>" onfocus="if(this.value=='Any') this.value='';">
         	</div>
         </div>
         <div>
         	<div Class="TwoColumnCardColumn1">Problem: </div>
         	<div Class="TwoColumnCardColumn2">
         		<input Class="TwoColumnCard" type="text" name="Problem" value="Any" size="<%=intInputSize%>" onfocus="if(this.value=='Any') this.value='';">
         	</div>
         </div>
         <div>
         	<div Class="TwoColumnCardColumn1">Notes: </div>
         	<div Class="TwoColumnCardColumn2">
         		<input Class="TwoColumnCard" type="text" name="Notes" value="Any" size="<%=intInputSize%>" onfocus="if(this.value=='Any') this.value='';"><input type="submit" value="Submit" style="float: right">
         	</div>
         </div>
         </form>
      </div>    
      <div class="TwoColumnCard">
         <form method="post" action="index.asp">
         <div class="TwoColumnCardTitle">Jump to Ticket</div>
         <div>
            <div Class="TwoColumnCardColumn1">Number </div>
            <div Class="TwoColumnCardColumn2"><input Class="TwoColumnCard" type="text" name="ID" size="<%=intInputSize%>">
            <input type="submit" value="Submit" name="Submit" style="float: right"> </div>
         </div>
         </form>
      
   <% If strTicketCheck <> "" Then %>
         <div Class="TwoColumnCardMerged"><font class="missing"><%=strTicketCheck%></font></div>
   <% End If %>
		</div>
   ...
   <% If strRole <> "Data Viewer" AND Application("UseAD") Then %>       
      <div class="TwoColumnCard">
         <form method="POST" action="index.asp">		
         <div class="TwoColumnCardTitle">Enter a New Ticket</div>

         <div>
         	<div Class="TwoColumnCardColumn1">Username: </div>
         	<div Class="TwoColumnCardColumn2">
         		<input Class="TwoColumnCard" type="text" id="usernames" name="UserName" value="<%=strUserName%>" size="15">
               <input type="submit" value="New Ticket" name="Submit" style="float: right">
            </div> 
         </div> 
         </form>
		<% If strNewTicketMessage <> "" Then %>
			<% If strNewTicketMessage = "Submitted" Then %>
				<div Class="TwoColumnCardMerged"><font class="information"><%=strNewTicketMessage%></font></div>
			<% Else %>
				<div Class="TwoColumnCardMerged"><font class="missing"><%=strNewTicketMessage%></font></div>
			<% End If %>
		<% End If %>
      </div>
   <% End If %>
...
<%    End If %> 
		<div>
      	<ul class="NavBar" align="center">
         <% If Application("UseStats") Then %>
               <li><a href="mstats.asp">Stats</a></li> 
         <% End If %>
  					<li><a href="settings.asp">Settings</a></li> 
  			<% If bolShowLogout Then %>
   				<li><a href="login.asp?action=logout">Log Out</a></li>
   		<% End If %>
  			</ul>
         ...
         <% If IsTablet Then %>
         	<div>
         		<form method="POST" action="index.asp">
               <input type="submit" value="Full Site" name="Submit">
            <% If Request.Cookies("ZoomLevel") = "ZoomIn" Then%>
